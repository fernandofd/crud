...
if (nomepag2 != undefined) {
    ...
    function listarusuarios0() {
        if  (conheceadmin == 0) {
            const confirmadm = prompt("Favor inserir a senha do administrador")
            if (confirmadm == user[0].senha) {
                conheceadmin = 1
            } else {
                alert("Acesso negado")
                location.reload();
        ...
        if(conheceadmin==1){
            ...
            for (let index = 0; index < user.length; index++) {
                lista.innerHTML += `Usuário ${index + 1} - ${user[index].nome} <br>`
            }
        }
    ...
    function adicionarusuarios1() {
        ...
        if (conheceadmin == 1) {
            ...
            lista.innerHTML += `<form>
                        ...
                        <input type="password" class="inputsenha2">
    ...
    function atualizarusuarios2() {
        ...
        let repetida = 0 // erro de usuário 
        ...
        for (let index = 0; index < user.length; index++) { // varredura nos nomes dos usuários para testar duplicidade
            if (user[index].nome == nome1) { // usado para teste se já existe nome na lista
                if (user[index].nome != nome) { // usado para permitir o mesmo nome e trocar somente a senha
                    repetida = 1  // usado para descrever erro de nome usuário
                }
            }
        }
        if (repetida == 0) { // somente executar se não tiver erro anterior
            for (let index = 0; index < user.length; index++) { // loop de teste de usuário
                if (user[index].nome == nome) { // testa se o nome digitado é valido no sistema para liberar a alteração
                    repetida = 0 // se sim zero a variável de erro afinal se achar uma vez ta correto
    ...
    botao.addEventListener("click", function () {
        ...
        for (let index = 0; index < user.length; index++) {
            if (nome == user[index].nome && senha1 == user[index].senha) {
                alert("Acesso liberado")
                // enviando o nome do usuário validado para o navegador
                localStorage.setItem("usuarioLogado", user[index].nome)
                index = user.length
                controle = 1
